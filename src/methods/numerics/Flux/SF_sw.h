...
/// @file SF_sw.h
/// @brief Steger-Warming通量矢量分裂与一阶面通量接口。
///
/// 该模块是用户配置`flux = "StegerWarming"`对应的Flux层实现。
/// WENO层调用`split()`进行正/负通量分裂；低阶回退或单元测试可调用
/// `flux()`/`storeFaceFlux()`使用一阶左右常值面通量。
...
namespace SF {
namespace Flux {
namespace StegerWarmingFlux {

/// @brief Steger-Warming正/负通量分裂。
...
/// @brief 计算指定面的Steger-Warming通量并写入FluxField。
/// @param field 结构网格场。
/// @param i 面左侧单元的i索引。
/// @param j 面左侧单元的j索引。
/// @param k 面左侧单元的k索引。
...
/// @brief 计算全场三个方向的一阶Steger-Warming通量。
