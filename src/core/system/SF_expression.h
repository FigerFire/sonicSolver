...
/// @brief 构造数学源项；数值 treatment 由 recipe 决定。
inline Term source(Symbol value) {
    return {TermKind::Source, std::move(value), {}};
}

/// @brief 构造边界或代数约束项；约束由边界/离散层执行，不伪装成体源。
inline Term constraint(Symbol value) {
    return {TermKind::Constraint, std::move(value), {}};
}

/// @brief 构造不经过 PDE 离散的代数关系。
inline Term algebraic(Symbol value) {
    return {TermKind::AlgebraicRelation, std::move(value), {}};
}
